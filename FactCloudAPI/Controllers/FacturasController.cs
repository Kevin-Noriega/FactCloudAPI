    ...
    public class FacturasController : ControllerBase
    {
        ...
        public async Task<ActionResult<Factura>> CrearFactura(Factura factura)
        {
            try
            {
                ...
                await _suscripcionService.IncrementarDocumentosUsados(factura.UsuarioId);
        ...
        public async Task<IActionResult> EnviarFacturaAlCliente(int id)
        {
            ...
            {
                return BadRequest(new { mensaje = "Error al enviar factura", error = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> EnviarADIAN(int id)
        {
            ...
            try
            {
                ...
                return Ok(new { mensaje = "Factura enviada exitosamente a DIAN", fechaEnvio = factura.FechaEnvioDIAN });
            }
            catch (Exception ex)
            {
        ...
        {
            if (pago == null) return BadRequest("Datos de pago inválidos");
            ...
            {
                _logger.LogError(ex, "Error generando XML factura {Id}", id);
                factura.XmlBase64 = null;
            }
        ...
        [HttpPatch("{id}")]
        public async Task<IActionResult> ActualizarEstado(int id, FacturaPagoDto dto)
        {
            var factura = await _context.Facturas
                .Include(f => f.Cliente)
                .FirstOrDefaultAsync(f => f.Id == id);
            ...
            await _context.SaveChangesAsync();
            return Ok(new { factura.Id, factura.Estado, factura.Cufe });
        }
        ...
        public async Task<IActionResult> DescargarXml(int id)
        {
            var factura = await _context.Facturas.FindAsync(id);
            if (factura == null) return NotFound();
            if (string.IsNullOrEmpty(factura.XmlBase64)) return BadRequest("XML no generado");

            byte[] bytes;
            try { bytes = Convert.FromBase64String(factura.XmlBase64); }
            catch { return BadRequest("XML Base64 inválido"); }

            return File(bytes, "application/xml", $"Factura_{factura.NumeroFactura}.xml");
        }

        private bool FacturaExists(int id) => _context.Facturas.Any(e => e.Id == id);
    }
}