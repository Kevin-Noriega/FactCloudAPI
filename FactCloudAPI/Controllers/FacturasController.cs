...
namespace FactCloudAPI.Controllers
{
    ...
    public class FacturasController : ControllerBase
    {
        ...
        public FacturasController(ApplicationDbContext context, IEmailService emailService, IHubContext<NotificacionesHub> hub, ILogger<FacturasController> logger)
        {
            _context = context;
    ...
    [FromQuery] DateTime? fechaFin)
        ...
        public async Task<IActionResult> RegistrarPago(int id, [FromBody] FacturaPagoDto pago)
        {
            ...
            factura.FechaPago = pago.FechaPago != default
                ? pago.FechaPago
                : DateTime.Now;
        ...
        public async Task<IActionResult> DescargarXml(int id)
        {
            ...
            if (string.IsNullOrEmpty(factura.XmlBase64))
                return BadRequest("XML no generado");

            byte[] bytes;

            try
            {
                bytes = Convert.FromBase64String(factura.XmlBase64);
            }
            catch
            {
                return BadRequest("XML Base64 inválido");
            }
